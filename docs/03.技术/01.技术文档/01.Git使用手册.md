...
当团队合作开发的过程中，线上环境master可能要合成dev中的部分功能，也可以理解为挑拣合成，这时候git cherry-pick就派上用场了

```bash
git cherry-pick commitId
```

到了这步的话，会出现两种情况：①第一种cherry-pick成功，然后把本地的push到远程服务器；②第二种cherry-pick发生冲突，这时需要手动解决冲突后再次提交至远程服务器；
